...
- 大标题“新发现”。
- 顶部 `SearchField`，用于本页内搜索。
- 无搜索词时显示新诗单、新诗精选和浏览题材。
- 有搜索词时显示 `SearchResultsView` 或空状态。

这个页面对应 Apple Music “Browse / New” 的模式，负责探索集合、题材和即时检索。
...
资料库的交互是“快速进入某类内容的第一个可读项”，当前没有单独的二级列表页；点击分类入口会打开对应范围的首个作品或首个诗单。
